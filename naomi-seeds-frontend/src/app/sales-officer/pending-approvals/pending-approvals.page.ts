...
export class PendingApprovalsPage implements OnInit, OnDestroy {
  ...
  filter = {
    ...
    count: 10,
    search: '',
    fromDistributorApproval: 'APPROVED',    // PENDING, APPROVED
    managerApprovalGenerateDC: 'APPROVED',  // PENDING, APPROVED
    salesOfficerApprovalStatus: 'PENDING',  // PENDING, SHIPPED
  ...
  async getAllSalesOrders() {
    ...
    this.challansSub = await this.salesService.getIptBySalesPerson(this.filter).subscribe(res => {
