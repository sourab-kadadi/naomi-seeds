...
export class DetailSalesOrderPage implements OnInit {
  ...
  inActive = false;
  profileId: any;
  stageTwoApprovalDistributor = false;
  stageFourApprovalDistributor = false;
    ...
    this.route.paramMap.subscribe((paramMap) => {
      ...
      if (res.status === false){
        ...
        return;
            ...
            if (this.profileId === res.data.fromDistributorId) {
              this.stageTwoApprovalDistributor = true;
            }
            if (this.profileId === res.data.toDistributorId) {
              this.stageFourApprovalDistributor = true;
            }
          },
          (error) => {
            this.isLoading = false;
  ...
  // getAllSalesOrder() {
  //   this.salesService.getChallanById('625026a87286ff04da7b980f').subscribe(res => {
  ...
  // async function handleButtonClick(ev: any) {
  ...
  onApproveByFromDistributor() {
    this.isLoading = true;
    this.inActive = true;
    this.ngxUiLoader.startLoader('loader-detail');
    this.salesService
      .approveByFromDistributor(this.salesOrderId)
      .subscribe(
        (res) => {
  ...
  onApproveByToDistributor() {
    this.isLoading = true;
    this.inActive = true;
    this.ngxUiLoader.startLoader('loader-detail');
    console.log(this.salesOrderId);
    this.salesService
      .approveByToDistributor(this.salesOrderId)
      .subscribe(
        (res) => {
          this.approved = res.data;
          this.isLoading = false;
