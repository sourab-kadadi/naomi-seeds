...
export class ProductsCategoryPage implements OnInit {
  ...
  async getAllProductCategories(init?: boolean, infinitScrollEvent?: any) {
    if (init === true) {
      this.ngxUiLoader.startLoader("loader-product-category-list");
      this.isLoading = true;
      this.active = false;
    } 
    this.productsCategoryService.getProductsCategory(this.filter).subscribe({
      ...
      error: (e) => {
        this.ngxUiLoader.stopLoader("loader-product-category-list");
  ...
  //   setTimeout(() => {
  //     if (this.productsCategoryList.length === this.totalCount) {
