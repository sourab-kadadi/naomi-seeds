...
export class CreateSalesOrderPage implements OnInit, OnDestroy {
  ...
  modelForm: any = this.formBuilder.group({
    ...
    driverName: [''],
    driverContact: ['', [Validators.pattern('^((\\+91-?)|0)?[0-9]{10}$')]],
  ...
  toDistributorChange(toevent: {
    ...
    // eslint-disable-next-line no-underscore-dangle
    // console.log('modelform', this.modelForm.fromDistributorId);
    // if( toevent.value._id === this.modelForm.fromDistributorId) {
    //   console.log(toevent.value._id, this.modelForm.fromDistributorId);
    //   this.presentAlert(MessageLib.FROM_TO_DISTRIBUTOR_ERROR);
    // } else {
    //   console.log(toevent.value._id, this.modelForm.fromDistributorId, 'nottriggeed');
    this.modelForm.patchValue({
      // eslint-disable-next-line no-underscore-dangle
  ...
  async onCreateChallan() {
    this.addItem();
    this.ngxUiLoader.startLoader('loader-create-salesOrder');
    this.challanService.createSalesByPlantManager(this.modelForm.value).subscribe((res) => {
      // loadingEl.dismiss();
      this.ngxUiLoader.stopLoader('loader-create-salesOrder');
      if (res.status === false) {
        ...
        this.router.navigate(['/plant-manager/sales-order']);
