...
export class SalesOrderPage implements OnInit, OnDestroy {
  ...
  async getAllSalesOrders() {
    this.isLoading = true;
    this.inActive = true;
    this.ngxUiLoader.startLoader('loader-sales-manager');
    this.challansSub = await this.salesService.getIptAll(this.filter).subscribe(res => {
      this.ngxUiLoader.stopLoader('loader-sales-manager');
      if(res.data) {
